...
void Bp::train()
{
	...
	for (int j = 0; j < train_set.size(); j++)
	{
		for (int i = 0; i < hidenode; i++)
		{
			wHnode[i][0] += rate * allerror[j] * hidevalueM[j][i];
		}
	}
	...
	for (int j = 0; j < train_set.size(); j++)
	{
		for (int i = 0; i < hidenode; i++)
		{
			hiderror[i] = allerror[j] * wHnode[i][0] * (1 - hidevalueM[j][i]) * hidevalueM[j][i];
		}
		allhiderror.push_back(hiderror);
	}
	...
	for (int k = 0; k < train_set.size(); k++)
	{
		for (int i = 0; i < innode; i++)
		{
			for (int j = 0; j < hidenode; j++)
			{
				wInode[i][j] += rate * allhiderror[k][j] * train_set[k][i];
			}
		}
	}
	*/
...
void Bp::init(vector<vector<double>> train_set, double error, double rate)
{
...
double Bp::test(vector<double> test)
{
	return 0.0;
}
